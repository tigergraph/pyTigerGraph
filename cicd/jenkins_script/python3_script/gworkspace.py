...
def main(parameters):
    ...
    util.run_bash(
        "sudo rm -rf /tmp/cmake_build; [ -d {}/cmakd_build ] && mv {}/cmake_build /tmp; sudo rm -rf {} || true".format(product_dir,product_dir,product_dir)
    )
    ...
    if not os.path.isdir(product_dir) or not os.path.isfile(product_dir + '/gworkspace.sh'):
        ...
        util.run_bash(
            "GIT_TRACE=1 git clone --progress https://{}:{}@github.com/tigergraph/product.git {} &>> {}".format(git_user, git_token, product_dir, log_file)
        )
