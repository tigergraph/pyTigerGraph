#!/usr/bin/python3
from .util import send_http_request, GithubAPIFail,AdvanceTagFail,decode_token,OpenIssueFail
import requests, json, time, re, sys, os, os.path, base64, pathspec
...
def send_github_api_request(url_tail, method='GET', data = None, silent = False,
    ...
    headers = {
        'Accept' : 'application/vnd.github.black-cat-preview+json, application/vnd.github.luke-cage-preview+json, application/vnd.github.v3+json',
...
# end class Enum
STATE = Enum(['APPROVE', 'REQUEST_CHANGES', 'COMMENT', 'DISMISS'])
REVIEW_STATE = { 'APPROVE': 'APPROVED', 'REQUEST_CHANGES': 'CHANGES_REQUESTED', 'COMMENT': 'COMMENTED', 'DISMISS': 'DISMISSED' }

########### pull reqest api ###########
def get_pull_request_closed(repo, base_branch):
    url_tail = repo + '/pulls?state=closed&per_page=100&base=' + base_branch
...
def get_file_content(repo, branch, loc):
    ...
    if "message" in response:
        raise GithubAPIFail(response.get('message') + " when fetching " + url_tail)
    else:
        return str(base64.b64decode(response['content']), "utf-8")
...
def get_codeowners_rules(repo, branch):
    ...
    # QA-2698
    rule_list_raw = codeowners_response.splitlines()
...
def get_pull_request_approvers(repo, num):
    ...
    for i in range(1, total_num + 1):
        ...
        for review in reviews:
            ...
            state = review['state']
            #user is not qa
            if state == REVIEW_STATE[STATE.REQUEST_CHANGES] and user != qa_account and changes_requested.count(user) == 0:
                changes_requested.append(user)
            elif state == REVIEW_STATE[STATE.APPROVE] and user != qa_account:
                approved.append(user)
                try:
                    changes_requested.remove(user)
                except ValueError:
                    pass
            elif state == REVIEW_STATE[STATE.DISMISS] and user != qa_account:
                try:
...
def get_review_id_by_user(repo, num, review_user, state, silent = False):
    reviews_res = get_pull_request_reviews(repo, num, 1)
    total_num = 1
    if not review_user:
    ...
    for i in range(1, total_num + 1):
        ...
        for review in reviews:
            user = review['user']['login']
            if user == review_user and review['state'] == REVIEW_STATE[state]:
                review_id = review["id"]
        if i < total_num:
...
def dismiss_comment_to_pull_request(repo, num, msg, review_id, silent = False):
    if num.isdigit():
        url_tail = repo + '/pulls/' + num + '/reviews/' + str(review_id) + "/dismissals"
        if not msg:
            msg = "Information updated"
        data = {"message":msg, "event":"DISMISS"}
...
def push_comment_to_pull_request(repo, num, msg, state, silent = False):
    if num.isdigit():
        qa_review_id = get_review_id_by_user(repo, num, "", state, silent = silent)
        if qa_review_id:
            ...
            if state == STATE.DISMISS:
                data = { "body": msg, "event": STATE.REQUEST_CHANGES}
                response = send_github_api_request(url_tail, 'POST', json.dumps(data), silent = silent)
                if response['id']:
                    response = dismiss_comment_to_pull_request(repo, num, "", response['id'])
                return response
            else:
...
def merge_pull_request(repo, num, url, sha):
    ...
    if response.get('merged', False) != True:
        raise GithubAPIFail(response.get('message'))
    else:
...
def send_github_api_request_ss(url_tail, retry, retry_interval):
  sys.stdout = open(os.devnull, 'w')
  
  try:
    content = send_github_api_request(url_tail, retry=retry, retry_interval=retry_interval)
  except:
    content = []
...
def get_tag(repo, tag, retry=3, retry_interval=1):
  url_tail = f"{repo}/git/ref/tags/{tag}"

  content = send_github_api_request_ss(url_tail, retry, retry_interval)
  
  return content

def is_tag_exists(repo, tag, retry=3, retry_interval=1):
  if not tag:
    return True

  content = get_tag(repo, tag, retry, retry_interval)
...
def get_branch(repo, branch, retry=3, retry_interval=1):
  url_tail = f"{repo}/branches/{branch}"

  content = send_github_api_request_ss(url_tail, retry, retry_interval)
  
  return content

def is_branch_exists(repo, branch, retry=3, retry_interval=1):
  if not branch:
    return True

  content = get_branch(repo, branch, retry, retry_interval)
...
def get_repo_tags(repo, is_recursive=False, retry=3, retry_interval=1):
  url_tail = f"{repo}/tags"

  content = send_github_api_request_ss(url_tail, retry, retry_interval)

  tags = {d["name"] for d in content}

  return tags

def get_repo_branches(repo, is_recursive=False, retry=3, retry_interval=1):
  url_tail = f"{repo}/branches"

  content = send_github_api_request_ss(url_tail, retry, retry_interval)

  branches = {d["name"] for d in content}

  return branches

def get_test_ids(test_loc, repo, test_type="", tag="", is_recursive=False, retry=3, retry_interval=1):
  """
  Args:
    repo: repository where tests located
    test_type: folder name that contains tests
    tag: repo tag name
  Returns:
    set
  """
  if repo not in test_loc:
    raise Exception("repo not found")

  if isinstance(test_loc[repo], dict):
    if test_type not in test_loc[repo]:
      raise Exception("type not found")

  if isinstance(test_loc[repo], dict):
    path = f"{test_loc[repo][test_type]}/{test_type}"
  else:
    path = f"{test_loc[repo]}/{test_type}"

  if is_recursive:
    url_tail = f"{repo}/git/trees/{tag}?recursive=1"
    raise NotImplementedError
  else:
    query = f"?&ref={tag}" if tag else ""
    url_tail = f"{repo}/contents/{path}{query}"

  content = send_github_api_request_ss(url_tail, retry=retry, retry_interval=retry_interval)
  
  test_ids = set()
  if content:
    test_ids = {re.search(r'regress(\d+)$', c['path']) for c in content}
    test_ids = {t.group(1) for t in test_ids if t}
...
def enable_branch_approval(repo, branch_name):
    ...
    enforce_owner = [ "gle", "blue_features", "gse" ]
    ...
    github_token = decode_token(fake_token)
    print(github_token)
...
def tag_branch_as_stable(repo, sha, tag_name):
    ...
    if "message" in response and response.get("message") != "Reference already exists":
        ...
        raise AdvanceTagFail(response.get('message'))
    # end if
...
def open_issue(repo, title, body, labels, **kwargs):
    ...
    if "message" in response:
        raise OpenIssueFail(response.get('message'))
    # end if
