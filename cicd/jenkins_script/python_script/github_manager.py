...
def get_commits_dict(input_format, input_content, repo_list_file, method):
    ...
    elif input_format == 'branch' or 'tag' or 'params':
        ...
        if method == 'create' or method == 'create_release':
            ...
            if method == 'create_release' and repo == 'product':
                ...
                util.run_bash("""
                    ...
                    version=$(echo '{}' | grep -e grep -e '^tg_[0-9]\+\.[0-9]\+\(\.[0-9]\+\)\?_dev$' | cut -d'_' -f2) && [ -n "$version" ] && echo "$version" > product_version && git add product_version
