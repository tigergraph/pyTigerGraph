...
class PyTigerGraphVertexBase():
    ...
    def _prep_upsert_vertex_dataframe(self, df, v_id, attributes):
        json_up = []
    ...
    def _prep_get_vertices(self, vertexType: str, select: str = "", where: str = "",
    ...
    def _prep_get_vertices_by_id(self, vertexIds, vertexType):
    ...
    def _parse_get_vertex_stats(self, responses, skipNA):
    ...
    def _prep_del_vertices(self, vertexType, where, limit, sort, permanent, timeout):
    ...
    def _prep_del_vertices_by_id(self, vertexIds, vertexType, permanent, timeout):
        '''url builder and param parser for delVerticesById()'''
        if not vertexIds:
            raise TigerGraphException("No vertex ID was specified.", None)
        vids = []
        if isinstance(vertexIds, (int, str)):
            vids.append(self._safe_char(vertexIds))
        else:
            vids = [self._safe_char(f) for f in vertexIds]
